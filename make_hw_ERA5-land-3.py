...
def process_file(file_path, var_name, y, resample=False):
    ...
    if resample and var_name=='hfls':
        ds['hfls'] = ds['hfls']*21600
        ds = ds.resample(time='D').mean('time') / 86400 
    if resample and var_name=='hfss':
        ds = ds.resample(time='D').mean('time') / 86400 
...
# Process hfss files
hfss_files = []
hfss_files.extend(glob.glob(f'{hfss_dir}/*_{y}_0[6-8].nc'))
...
ds_hfss_regrid = regrid(ds_hfss,ds_tasmax,method)
ds_hfss = ds_hfss_regrid
# - - - - - - - - - - - - - - - - -
...
ds_hfss = ds_hfss.chunk({"time": -1, "lat": 10, "lon": 10}).persist()
...
P1=axs.pcolormesh(ds_tasmax.lon,ds_tasmax.lat,ds['mean_evaporative_fraction'],transform=ccrs.PlateCarree())
