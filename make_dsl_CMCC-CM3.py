...
cam_lsm=      str(sys.argv[8])
models=           str(sys.argv[9])
f_out=            str(sys.argv[10])

# In[3]:
cam_lsm=xr.open_dataset(f'{cam_lsm}')
ds_mod1 = xr.open_mfdataset(f'{pr_dir}/pr_CMCC-CM3_BlueAdapt_HISTORICAL*.nc')
ds_mod1 = ds_mod1.sel(time=ds_mod1['time'].dt.month.isin([6, 7, 8]))
ds_mod2 = xr.open_mfdataset(f'{pr_dir}/pr_CMCC-CM3_BlueAdapt_SSP585_6hr*.nc').resample(time='D').sum()*21600000
ds_mod2 = ds_mod2.sel(time=ds_mod2['time'].dt.month.isin([6, 7, 8]))
ds_pr=xr.concat([ds_mod1,ds_mod2],dim='time')
#cdo -mulc,21600000 -daysum tmp.t02.nc tmp2.t02.nc
...
nlat, nlon = ds_pr.pr.shape[1], ds_pr.pr.shape[2]
...
out_index_event_start = np.full(([len(range(ystart,yend)),ds_pr.pr.shape[1],ds_pr.pr.shape[2]]),np.nan)
out_index_event_end   = np.full(([len(range(ystart,yend)),ds_pr.pr.shape[1],ds_pr.pr.shape[2]]),np.nan)
out_mean              = np.full(([len(range(ystart,yend)),ds_pr.pr.shape[1],ds_pr.pr.shape[2]]),np.nan)
out_max               = np.full(([len(range(ystart,yend)),ds_pr.pr.shape[1],ds_pr.pr.shape[2]]),np.nan)
out_number            = np.full(([len(range(ystart,yend)),ds_pr.pr.shape[1],ds_pr.pr.shape[2]]),np.nan)

# Iterate over years
for year_idx, year in enumerate(years):
    cc=cc+1
    print(f"Processing model: {models}, year: {year}")
    pr_y = ds_pr.sel(time=ds_pr.time.dt.year == year).pr.where(mask == 0, np.nan).values
