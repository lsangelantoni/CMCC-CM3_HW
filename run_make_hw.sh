...
python /users_home/cmcc/ls21622/CMCC-CM3_HW/scripts/python/hw/make_all_days_hw.py "$ystart" "$yend" "$lon_min" "$lon_max" "$lat_min" "$lat_max" "$tx_dir" "$p90_dir" "$hfls_dir" "$cam_lsm_dir" "$models" "$f_out"
